...
async def buy(amount, token_address):
...
async def main(action, amount, token_address):
    if action == "buy":
        await buy(amount, token_address)
    elif action == "sell":
        await sell(amount, token_address)
    else:
        print("Action không hợp lệ. Vui lòng sử dụng 'buy' hoặc 'sell'.")

if __name__ == "__main__":
    if len(sys.argv) < 4:
        print("Usage: python bot.py [buy/sell] [amount] [token_address]")
        sys.exit(1)
